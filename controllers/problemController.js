...
exports.getProblemBySlug = catchAsyncErrors(async (req, res, next) => {
  ...
  // Filter test cases - only show samples to non-authenticated users
  if (!userId) {
    problem.testCases = problem.testCases.filter(tc => tc.isSample);
  } else if (userId.toString() !== problem.createdBy._id.toString()) {
    // Hide hidden test cases from non-authors
    problem.testCases = problem.testCases.filter(tc => tc.isSample || !tc.isHidden);
  }

  // Add user's submission status if authenticated
  if (userId) {
    const submission = await Submission.findOne({
      userId,
      problemId: problem._id,
...
exports.createProblem = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!hasSample) {
    return next(new ErrorHandler('At least one sample test case is required', 400));
  }
